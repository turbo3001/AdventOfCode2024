...
class PathNode:
    def __init__(self, Number:int):
        ...
        self.PathEnds:list[PathNode]|None = None
    ...
    def GetUniquePathEnds(self) -> list:
        if self.PathEnds is None:
            self.PathEnds = []
            if self.Number == 9:
                self.PathEnds.append(self)
            else:
                for Dir in NodeDir:
                    NextNode:PathNode|None = self.NextNodes[Dir]
                    if NextNode is not None:
                        self.PathEnds += NextNode.GetUniquePathEnds()
        return list(set(self.PathEnds))
    ...
    with open("./Day_10/InputData/Input.txt") as file:
        ...
        for MapRow in TrailMap:
            for MapNode in MapRow:
                if MapNode.Number == 0:
                    NodeScore:int = len(MapNode.GetUniquePathEnds())
                    print(NodeScore)
                    sum += NodeScore
        print(sum)
