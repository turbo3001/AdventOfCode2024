    ...
    def CalculateValidAntiNodes(self) -> list[Vector]:
        ...
        for NodeChar in self.Nodes:
            ...
            for NodePair in combinations(NodeList, 2):
                if not ListContainsPoint(validAntiNodes, NodePair[0]):
                    validAntiNodes.append(NodePair[0])
                if not ListContainsPoint(validAntiNodes, NodePair[1]):
                    validAntiNodes.append(NodePair[1])
                
                DiffPoint:Vector = NodePair[0] - NodePair[1]
                NewPoint:Vector = NodePair[0] + DiffPoint
                while self.ContainsPoint(NewPoint):
                    if not ListContainsPoint(validAntiNodes, NewPoint):
